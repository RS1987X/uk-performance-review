...
def load_transactions(formatted_tx_csv: Path) -> pd.DataFrame:
    ...
    tx["Portfolio"] = tx["Portfolio"].fillna("").astype(str).str.upper()
...
def load_start_positions(start_pos_csv: Path) -> pd.DataFrame:
    ...
    if start_pos_csv.exists():
        ...
        start_df["Portfolio"] = start_df["Portfolio"].astype(str).str.upper()
        # Filter out blank portfolio lines
        return start_df[start_df["Portfolio"] != ""]
    else:
        return pd.DataFrame(columns=["Portfolio", "Name", "Shares", "ccy"])
